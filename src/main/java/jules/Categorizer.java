...
public class Categorizer {
	...
	public static List<Pair<String, Double>> getCategories(String q) {
		...
		try {
			 ...
			 * System.out.println(categories); System.out.println(decvals);
			 */
			...
			for (int i = 0; i < cats.length; i++) {
				...
				if (!cats[i].isEmpty()) {
					try {
						Pair<String, Double> pa = new Pair<String, Double>(cats[i],
								((new BigDecimal(vals[i]).doubleValue()) / maxAbs + 1) / 2);
						prs.add(pa);
					} catch (Exception e) {
